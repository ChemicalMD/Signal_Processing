...
Measurement_Controls = [sg.Frame("Measurement Controls", expand_x=True, layout=[
        [sg.Button('Select Measurement', key='-OPENDATA-')],
...
Reference_Controls = [sg.Frame("Reference Controls", expand_x=True, layout=[
        [sg.Button('Select Reference', key='-OPENREF-')],
        [sg.Text('File path:', key='-PATHREF-')],
        [sg.Text('File name:', key='-NAMEREF-')],
        [sg.Column([[sg.Frame('Read which columns? (X,Y)', layout=[
                    [sg.Text('('), sg.Input(size=(5,1), key='-XCOLREF-'), sg.Text(','), sg.Input(size=(5,1), key='-YCOLREF-'), sg.Text(')')],
                    ])]]),
        sg.Column([[sg.Frame('Skip how many rows?', layout=[
                    [sg.Input(size=(5,1), key='-SKIPROWSREF-')],
                    ])]]),
        sg.Column([[sg.Frame('X-Limits (low,high)', layout=[
                    [sg.Text('('), sg.Input(size=(5,1), key='-XLOWREF-'), sg.Text(','), sg.Input(size=(5,1), key='-XHIGHREF-'), sg.Text(')')],
                    ])]])]])]

Plot_Controls = [sg.Frame("Plot Controls", expand_x=True, layout=[
        [sg.Column([[sg.Frame('X-Limits (low,high)', layout=[
                    [sg.Text('('), sg.Input(size=(5,1), key='-XLOWPLOT-'), sg.Text(','), sg.Input(size=(5,1), key='-XHIGHPLOT-'), sg.Text(')')],
                    ])]]),
        sg.Column([[sg.Frame('Y-Limits (low,high)', layout=[
                    [sg.Text('('), sg.Input(size=(5,1), key='-YLOWPLOT-'), sg.Text(','), sg.Input(size=(5,1), key='-YHIGHPLOT-'), sg.Text(')')],
                    ])]])],
...
if True: # layout and window
    ...
    controls_col = sg.Column([
        ...
        Reference_Controls,
        Plot_Controls,], expand_y=True)
    
    figure_col = sg.Frame("", [[sg.Canvas(background_color='green', expand_x=True, expand_y=True, key='-CANVAS-')]], size=(800, 600))
    
    message_col = sg.Frame('Messages', expand_x=True, expand_y=True, layout=[[sg.Text('(Messages will appear here)', key='-MESSAGE-')]])
...
while True:
    ...
    if event == '-OPENDATA-':
        ...
        if re.split(r'[/,.]', data_path)[-1] in ['CSV','csv','txt']:
        ...
        if have_data:
            try:
                if (values['-XCOL-'] != '') & (values['-YCOL-'] != '') & (values['-SKIPROWS-'] != ''):
                    ...
                    window['-MESSAGE-'].update('Please fill in "Read which columns" and "Skip how many rows" in "Measurement Controls".')
                    continue
            except Exception as e:
            ...
            if (values['-XLOWPLOT-'] != '') & (values['-XHIGHPLOT-'] != ''):
                ax.set_xlim(float(values['-XLOWPLOT-']),float(values['-XHIGHPLOT-']))
            if (values['-YLOWPLOT-'] != '') & (values['-YHIGHPLOT-'] != ''):
                ax.set_ylim(float(values['-YLOWPLOT-']),float(values['-YHIGHPLOT-']))
    ...
    if event == '-OPENREF-':
        ...
        if re.split(r'[/,.]', ref_path)[-1] in ['CSV','csv','txt']:
    ...
    if event == '-PLOTREF-':
        if have_ref:
            try:
                if (values['-XCOLREF-'] != '') & (values['-YCOLREF-'] != '') & (values['-SKIPROWSREF-'] != ''):
                    Ref_Data = np.loadtxt(ref_path, delimiter=",", usecols=[int(values['-XCOLREF-']),int(values['-YCOLREF-'])], skiprows=int(values['-SKIPROWSREF-']))
                else:
                    window['-MESSAGE-'].update('Please fill in "Read which columns" and "Skip how many rows" in "Reference Controls".')
                    continue
            except Exception as e:
                window['-NAME-'].update('ERROR: ' + str(e) + ' (file: ' + data_path.split('/')[-1] + ')')
    ...
    if event == '-CORR-':
        if have_data & have_ref:
            ...
            if len(corr) < len(delay_array):
                corr = np.append(corr, np.zeros(len(corr) - len(delay_array)))
            ...
            window['-MESSAGE-'].update('Need both data and reference.')
    
    if event == '-OVERLAP-':
        if have_corr:
            #overlap(Reference, Waveform, arrival)
            
            Reference_X = [round(float(t),10) for t,v in zip(Ref_Data[:,0], Ref_Data[:,1])]
            Reference_Y = norm([v for t,v in zip(Ref_Data[:,0], Ref_Data[:,1])])
            
            X = Data[:,0]
            Y = norm(Data[:,1])

            ax.cla() # clear the current figure
            ax.plot([(t+arrival)*1e6 for t in Reference_X], norm(Reference_Y), color="blue", linestyle="-", label="Reference")
            ax.plot([t*1e6 for t in X], norm(Y), color="red", linestyle="-", label="Data")
            ...
            window['-MESSAGE-'].update('Cannot plot overlap before successful correlation. Click "Plot Correlation".')
